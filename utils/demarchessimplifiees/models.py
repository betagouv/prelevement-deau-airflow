...
class PieceJointe(Base):
    ...
    demarches_simplifiees_url = Column(String, comment="l'URL sur demarches simplifiees")
    ...
    dossier = relationship('PreprocessedDossier', secondary='dossier_fichier_tableau_suivi_camion_citerne_assoc')
...
class FichiersTableursAssoc(Base):
    ...
    donnees_point_de_prelevement_id = Column(UUID(as_uuid=True), ForeignKey('donnees_point_de_prelevement.id'),
                                             primary_key=True)
    piece_jointe_id = Column(UUID(as_uuid=True), ForeignKey('piece_jointe.id'), primary_key=True, comment="La clé primaire de la piece jointe")
...
class DemarcheDataBrute(Base):
    ...
    hashed_collected_data = Column(String, unique=True, comment="Le hash du fichier de snapshot.")
    object_storage_key = Column(String, unique=True, comment="La cle du fichier de snapshot dans le bucket.")
...
class PreprocessedDossier(Base):
    ...
    adresse_email_connexion = Column(String, comment="Email de connexion associé au compte Démarches simplifiées du déclarant")
    ...
    depot_pour_mandataire = Column(Boolean, comment="Indique si le déclarant dépose pour un mantataire")
    ...
    derniere_mise_a_jour = Column(DateTime, comment="Date de dernière mise à jour du dossier (notamment les avis, la messagerie…)")
    date_depot = Column(DateTime, comment="Date de dépôt du dossier")
    date_passage_instruction = Column(DateTime, comment="Date de passage en instruction")
    date_traitement = Column(DateTime, comment="Date de traitement par l’instructeur")
    motivation_decision = Column(String, comment="Motivation de la décision par l’instructeur")
    instructeurs = Column(String, comment="Instructeurs affectés à la démarche")
    groupe_instructeur = Column(String, comment="Groupe d’instructeurs affecté à la démarche")

    coordonnees = Column(String)
    adresse_email_declarant = Column(String, comment="Adresse email du déclarant")
    numero_telephone_declarant = Column(String, comment="Numéro de téléphone du déclarant")
    statut_declarant = Column(String, comment="Statut du déclarant (particulier ou représentant d’une structure)")
    ...
    mode_transmission_donnees_camion_citerne = Column(String, comment="Mode de transmission des données pour les AOT camion citerne (une à une, ou au format tableur)")
    volumes_pompes_jour = Column(String)
    copie_registre_papier = Column(String)
    conclusion = Column(String)
    commentaire = Column(String, comment="Commentaire libre du déclarant sur la déclaration")
    ...
    numero_serie_compteur = Column(String, comment="Numéro de série du compteur (pour les AOT agricoles)")
    ...
    validation_informations = Column(Boolean, comment="Déclaration par le préleveur que les informations sont exactes")
    details_prelevements_camion_citerne = Column(Boolean)
    donnees_compteur_volumetrique = Column(Boolean, comment="Indique sur les données sont issues d’un compteur volumétrique (pour les AOT agricoles)")
    compteur_lecture_directe = Column(Boolean, comment="Indique s’il s’agit d’un compteur à lecture directe (pour les AOT agricoles)")
    panne_compteur = Column(Boolean, comment="Indique si une panne ou un changement de compteur est déclarée (pour les AOT agricoles)")
    prelevement_sur_periode_aot_agricole = Column(Boolean)
    prelevement_sur_periode_camion_citerne = Column(Boolean, comment="Pour les AOT de camions citernes, indique si des prélèvements ont été réalisés sur la période concernée par la déclaration (mois précédent)")
    date_debut_periode_declaree = Column(DateTime, comment="Date du début de la période concernée par la déclaration")
    date_fin_periode_declaree = Column(DateTime, comment="Date de fin de la période concernée par la déclaration")
    annee_prelevement_camion_citerne = Column(Integer, comment="Pour les camions citernes, année de prélèvement (permet de définir le niveau de précision attendu dans la déclaration)")

    fichier_tableau_suivi_camion_citerne = relationship('PieceJointe', secondary='dossier_fichier_tableau_suivi_camion_citerne_assoc')
...
class ReleveIndex(Base):
    ...
    ligne = Column(Integer, comment="Ordre dans lequel l’index a été déclaré pour une même déclaration")
...
class VolumesPompes(Base):
    ...
    ligne = Column(Integer, comment="Ordre dans lequel l’index a été déclaré pour une même déclaration")
    point_prelevement_camion_citerne = Column(String, comment="Point de prélèvement pour le camion citerne")
    annee_prelevement_camion_citerne_2 = Column(Integer, comment="Année à laquelle le volume prélevé est associé (pour les prélèvements antérieurs à 2024, on accepte un chiffre globalisé à l’année)")
    volumes_pompes_camions_citernes = Column(Float, comment="Volumes pompés par les camions citernes (en m³), pour les déclarants ayant voulu déclarer leurs données une à une")
    date_prelevement_camion_citerne = Column(DateTime, comment="Date précise de prélèvement par camion citerne")
...
class Message(Base):
    ...
    id_dossier = Column(Integer, comment="Identifiant unique du dossier")
    id_message = Column(String, comment="Identifiant unique du messge dans demarches simplifiees")
